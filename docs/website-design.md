...
纯 HTML/CSS/原生 JavaScript，零 CDN、零构建依赖。GitHub Pages workflow 只上传 `site/` 静态目录；主页、审查页和资源均使用相对路径，兼容仓库子路径部署。所有外链、站内锚点、375/768/1440px 布局在真实浏览器验证。
...
5. 视觉复核后做第二轮打磨。
6. 提交并推送，启用 GitHub Pages，最后访问线上 URL 验证。
...
5. **GitHub Pages Actions 部署**：与代码同源、可审查、无第三方依赖；代价是静态站不提供服务端功能。
