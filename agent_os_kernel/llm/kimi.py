# -*- coding: utf-8 -*-
"""Kimi (Moonshot AI) Provider - Kimi API 支持
...
class KimiProvider(LLMProvider):
    ...
    def base_url(self) -> str:
        return self.config.base_url or "https://api.moonshot.cn/v1"
    
    async def initialize(self):
        """初始化 Kimi 客户端"""
        if not self.config.api_key:
            raise ValueError("Kimi API key is required. Set KIMI_API_KEY env variable.")
        ...
        logger.info(f"Kimi provider initialized with model: {self.config.model}")
